...
- `Yoink/MainWindow.axaml` / `MainWindow.axaml.cs` — the entire app UI and logic lives here, in code-behind (no MVVM/ViewModel layer — keep new features in this same style unless the UI grows enough to justify introducing one):
  ...
  - `DownloadVideoAsync` uses `YoutubeExplode` (`YoutubeClient`) to fetch the video's stream manifest, then picks the muxed MP4 stream whose height is closest to the requested resolution (YouTube stopped serving muxed streams above 720p, so 1080/1440 selections intentionally fall back to the best available muxed stream rather than failing). YoutubeExplode is only used to resolve that stream's direct URL and video metadata — the actual byte transfer goes through `DownloadEngine` (see below), not `YoutubeClient.Videos.Streams.DownloadAsync`. The file is saved into the app's own base directory (`AppContext.BaseDirectory`) using the video's title as the filename.
...
- `Yoink/DownloadEngine.cs` — the generic core download engine described in the README roadmap's step 1: a source-agnostic, resumable single-file HTTP downloader (range-request resume, progress via `IProgress<double>`, retry-with-backoff, cancellation). It writes to `<destination>.partial` and only moves the file into place on success, so a killed download resumes instead of restarting. `MainWindow` holds one instance for the app's lifetime (disposed in `OnClosed`, since it owns an `HttpClient` that should be reused rather than recreated per download) and `DownloadVideoAsync` hands it the resolved YouTube stream's direct URL — it's the layer that actually moves bytes for every download, YouTube or otherwise, that the app will do going forward.
